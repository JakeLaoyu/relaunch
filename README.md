...
- **Dock 图标** — 自带应用图标，默认显示在 Dock，点击即唤起启动台（可在设置中关闭）。
- **菜单栏图标** — 可在设置中开关是否显示；下拉菜单可打开启动台 / 设置 / 导入分组 / 退出。
- **设置窗口** — 启动台搜索行最右侧的 **`•••`** 按钮（或菜单栏「设置…」）打开，可开关：Dock 图标、菜单栏图标、捏合手势、快捷键、开机启动，以及导入旧版分组。
- **开机启动** — 基于 `SMAppService`（macOS 13+）。
- **文件夹与拖拽整理**
...
- **导入旧版启动台分组**：首次启动若没有自己的布局，会自动读取旧版 Launchpad 数据库（`<DARWIN_USER_DIR>com.apple.dock.launchpad/db`）还原页面顺序与文件夹；也可随时在设置窗口或菜单栏里手动重新导入。按 bundle id 匹配已安装应用，已卸载的自动忽略。
